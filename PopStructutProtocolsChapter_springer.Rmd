...
$3=="Sardinian"||$3=="Tuscan" {print $0}' raw_input/H938.clst.txt \
> out/Euro.clst.txt

plink --bfile raw_input/H938 --keep out/Euro.clst.txt --make-bed \
--out out/H938_Euro
```
...
```{r, eval = TRUE, fig.height=2, fig.width=4.5,fig.cap="Initial plot of the ADMIXTURE results for K=8 using R base graphics."}
library(RColorBrewer)
tbl <- read.table("out/H938_Euro.LDprune.8.Q")
par(mar = c(1.5, 4, 2.5, 2),cex.lab=0.75,cex.axis=0.6)
barplot(t(as.matrix(tbl)),
  col = brewer.pal(8, "Set1"), ylab = "Anc Proportions",
  border = NA, space = 0
)
```
...
![Plot of the ADMIXTURE results for K=8 using PONG.](./plot/H938_Euro_LDprune_K8.png){width=5.4inch}
...
Because ADMIXTURE is a "greedy", "hill-climbing"" optimization algorithm it is good practice to do multiple runs from differet initial random starting points.  We can do this by using the -s flag to specify the random seed for each ADMIXTURE run. 
...
# run pong
pong -m out/${prefix}.k6multiplerun.Qfilemap --greedy -s .9 \
-i out/H938_Euro.ind2pop
```


![PONG plot summarizing multiple ADMIXTURE runs with different random starting points.  The top row shows the major mode (supported by 8 out of 10 runs as indicated in the blue text).  The next two rows show two other solutions found by ADMIXTURE in 3 and 1 runs respectively.](./plot/modes_K=6_2018-10-12_10h6m16s_pong.png){width=5.4inch}

The resulting figure shows that 8 out of 10 runs converged to the same mode. Interestingly this is not the solution our single runs above found. For instance, the appearance of STRUCTURE within Sardinia is not stable across runs, and is missing from the major mode.  Another difference from our initial runn is the major mode models the North Italian and Tuscan samples as a unique cluster.  The third mode (supporting by just one run) inferred sub-structure within the French Basque sample. This instability in the solution is a hint that the ADMIXTURE model with K=8 is not a perfect fit to this data.  
...
![Example PONG output showing results from across a range of K values (with ten ADMIXTURE runs per K value).](./plot/mainviz_2018-10-12_10h7m3s_pong.png){ width=5.4inch}

Here we see how as $K$ increases through to $K=6$, the Sardinian, Basque, Adygei, and Russian samples are modeled as descended from unique sources, at $K$ of 7, 8, 9 reveal structure within the Sardinian, Orcadian, and Basque samples is revealed, though for each the substructure is not fully stable at higher values of $K$.  The values of $K=10$ and above make increasingly finer scale divisions that are difficult to interpret, and major modes only consist of one to three runs.  
...
```{r, eval = TRUE, echo=TRUE, results='hide',warning=FALSE,message=FALSE, fig.height=10, fig.width=10, out.width = "100%", fig.cap="Pairwise plots of PC values generated using the PCAviz package."}
library(PCAviz)
library(cowplot)
